...
*added by ilknurnacar 18052020
  AUTHORITY-CHECK OBJECT 'ZSOG_BUTCE'
           ID 'ACTVT' FIELD '01'.
  IF sy-subrc = 0.
    lv_budget = 'X'.
  ENDIF.
*  end of added ilknurnacar 18052020
      ...
      LOOP AT SCREEN.
        CASE screen-group1.
          ...
          WHEN 'M2'.
            ...
            MODIFY SCREEN.
          WHEN 'M10'.
*            IF gv_ucomm = 'U1' AND c_trans = 'X'.
            IF c_trans = 'X'.
              screen-invisible = 1.
              screen-active = 0.
*            ELSEIF gv_ucomm = 'U1' AND c_contr = 'X'.
            ELSEIF  c_contr = 'X'.
              screen-invisible = 0.
              screen-active = 1.
            ELSE.
...
*         mus_ozel    TYPE char50,                      " commented by ilknurnacar 06042020
         cargo_key   TYPE zsog_mm_019_t_03-cargo_key," added by ilknurnacar 06042020
  ...
  DATA: lt_eban TYPE TABLE OF gtt_eban,
        ls_eban TYPE gtt_eban.
...
*        ls_kargo1  TYPE zsog_mm_006_t_02.
  DATA: lt_kargo1  TYPE TABLE OF zsog_mm_019_t_02,    " added by ilknurnacar 06042020
        ls_kargo1  TYPE zsog_mm_019_t_02.

  DATA: lt_kargo2  TYPE TABLE OF ltt_kargo,
        ls_kargo2  TYPE ltt_kargo.

*  DATA: lr_mus_ozel TYPE RANGE OF zsog_mm_006_t_03-musteri_ozel_kodu " commented by ilknurnacar 06042020
*        WITH HEADER LINE.
  DATA: "lv_mus_ozel TYPE string,                                      " commented by ilknurnacar 06042020
  ...
  LOOP AT lt_eban INTO ls_eban.
    READ TABLE lt_talep INTO ls_talep
      WITH KEY matnr = ls_eban-matnr
               ...
               sil   = '' BINARY SEARCH.
    IF sy-subrc EQ 0.
* added by Prodea ilknurnacar-17022020- talep tarihi güncelleme
      PERFORM update_eban_badat USING    ls_talep
                                CHANGING ls_eban.
* end of added Prodea  ilknurnacar-17022020
...
*    SELECT * FROM zsog_mm_006_t_02 AS a          " commented by ilknurnacar 06042020
*       INTO TABLE lt_kargo1
*       FOR ALL ENTRIES IN lt_talep
*            WHERE matnr = lt_talep-matnr
*              AND afnam = lt_talep-kunnr
*              AND bedat IN lr_bedat.             " end of commented by ilknurnacar 06042020
    SELECT * FROM zsog_mm_019_t_02 AS a           " added by ilknurnacar 06042020
           INTO TABLE lt_kargo1
           FOR ALL ENTRIES IN lt_talep
                WHERE matnr = lt_talep-matnr
                  AND afnam = lt_talep-kunnr
                  AND bedat IN lr_bedat.           " end ofadded by ilknurnacar 06042020
  ENDIF.

  LOOP AT lt_kargo1 INTO ls_kargo1.

*    CONCATENATE ls_kargo1-banfn ls_kargo1-bnfpo ls_kargo1-afnam  " commented by ilknurnacar 06042020
*           INTO lv_mus_ozel.
...
*    ENDIF.                                         " end of commented by ilknurnacar 06042020
    lv_cargo_key = ls_kargo1-cargo_key.             " added by ilknurnacar 06042020
    ...
    ENDIF.                                             " end of added by ilknurnacar 06042020
  ENDLOOP.

*  SELECT * FROM zsog_mm_006_t_03 AS a                  " commented by ilknurnacar 06042020
*     INTO TABLE lt_kargo
*          WHERE a~musteri_ozel_kodu IN lr_mus_ozel
*            AND a~durum_kodu NE '6'.                  " end of commented by ilknurnacar 06042020
  SELECT * FROM zsog_mm_019_t_03 AS a                  " added by ilknurnacar 06042020
       INTO TABLE lt_kargo
            WHERE a~cargo_key IN lr_cargo_key
              AND a~err_code NE '5'.          " end of added by ilknurnacar 06042020
  LOOP AT lt_kargo INTO ls_kargo.

*    READ TABLE lt_kargo2 INTO ls_kargo2                commented by ilknurnacar 06042020
*    WITH KEY mus_ozel = ls_kargo-musteri_ozel_kodu.
    CLEAR ls_kargo2.
...
FORM siparis_durum_raporu .
  ...
  TYPES: BEGIN OF ltt_cargo,
        ebeln   TYPE  zsog_mm_019_t_02-ebeln, " zsog_mm_006_t_02 tablosu zsog_mm_019_t_02 olarak değiştirildi
        ...
        err_code   TYPE  zsog_mm_019_t_02-err_code,
*        DELIVERY_DATE
*DELIVERY_TIME
*RECEIVER_INFO
...
*        teslim_saati    TYPE zsog_mm_006_t_03-teslim_saati,
  ...
  TYPES: BEGIN OF ltt_teslimat,
         err_code TYPE zsog_mm_019_t_03-err_code, " added by ilknrnacar 06042020
*        durumu            TYPE  zsog_mm_006_t_03-durumu,           " commented by ilknrnacar 06042020
*        kargo_kodu        TYPE  zsog_mm_006_t_03-kargo_kodu,
*        teslim_alan       TYPE  zsog_mm_006_t_03-teslim_alan,
*        teslim_tarihi     TYPE  zsog_mm_006_t_03-teslim_tarihi,
*        teslim_saati      TYPE  zsog_mm_006_t_03-teslim_saati,
*        musteri_ozel_kodu TYPE  zsog_mm_006_t_03-musteri_ozel_kodu, " end of commented by ilknrnacar 06042020
       END OF ltt_teslimat.
...
*!! lt_cargookunmamış ilknurnacar06042020
...
*    ENDIF.
    ...
    IF sy-subrc EQ '0'.
      IF ls_cargo-err_code EQ '0'.
        ls_alv-cargo_durumu = 'Sipariş Kargoya iletildi'.
      ELSEIF ls_cargo-err_code EQ '1' OR ls_cargo-err_code EQ '2'.
        ls_alv-cargo_durumu = 'Sipariş Kargoya iletilmedi'.
      ENDIF.
    ENDIF.
...
*  IF gs_scr-1903-ucomm_first_alv EQ '&YENI'.
  ...
  IF rb6 = 'X' AND c_trans = 'X'.
    ls_fieldcat-no_out = 'X'.
    MODIFY gs_scr-1903-fieldcat FROM ls_fieldcat TRANSPORTING no_out
                                WHERE fieldname = 'KALAN_BUTCE'.
  ELSEIF rb6 = 'X' AND c_contr = 'X'.
    ls_fieldcat-no_out = 'X'.
    MODIFY gs_scr-1903-fieldcat FROM ls_fieldcat TRANSPORTING no_out
...
bapiebanc
  ...
  pt_requisition_items-c_amt_bapi = '1'.
...
FORM delline_1903 .
  ...
  DATA:
lt_items_to_delete TYPE STANDARD TABLE OF bapieband,
ls_items_to_delete LIKE LINE OF lt_items_to_delete ,
lt_return	 TYPE STANDARD TABLE OF bapireturn,
ls_return	 LIKE LINE OF lt_return .
  DATA: lt_return2 TYPE STANDARD TABLE OF bapireturn,
        ls_return2 LIKE LINE OF lt_return .

  DATA: lt_alv TYPE TABLE OF  zsog_mm_007_s_001.
  FIELD-SYMBOLS:<fs_alv2> TYPE zsog_mm_007_s_001.

  FIELD-SYMBOLS: <fs_alv> TYPE zsog_mm_007_s_001.

  ls_ui_popup_text-titlebar = 'Satırı Sil'.
  ls_ui_popup_text-question =
  'Kayıtlar Silinecektir! Onaylıyor musunuz?'.
  ...
  IF lv_answer EQ '1'.
    ...
    LOOP AT lt_alv ASSIGNING <fs_alv> WHERE sil IS NOT INITIAL.
      LOOP AT lt_alv ASSIGNING <fs_alv2> WHERE banfn = <fs_alv>-banfn.
        ls_items_to_delete-preq_item = <fs_alv2>-bnfpo.
        ls_items_to_delete-delete_ind = 'X'.
        ls_items_to_delete-closed = ' '.
        APPEND ls_items_to_delete TO lt_items_to_delete.
      ENDLOOP.
      IF sy-subrc EQ 0.
        CALL FUNCTION 'BAPI_REQUISITION_DELETE' DESTINATION 'NONE'
          EXPORTING
            number                      = <fs_alv>-banfn
          TABLES
        ...
        IF sy-subrc EQ 0.
          <fs_alv>-sil = ' '.
          MODIFY lt_alv FROM <fs_alv> TRANSPORTING sil WHERE banfn = <fs_alv>-banfn.
          CALL FUNCTION 'BAPI_TRANSACTION_COMMIT' DESTINATION 'NONE'
            EXPORTING
              wait = 'X'.
        ELSE.
        ...
        ENDIF.
      ENDIF.
      CLEAR: ls_rowindex, ls_items_to_delete, lt_items_to_delete,ls_return2.
    ENDLOOP.
...
*    MODIFY <grid_alv> FROM TABLE lt_alv.
*    <grid_alv> = lt_alv.
...
*    LOOP AT lt_rowindex INTO ls_rowindex.
*      READ TABLE <grid_alv> ASSIGNING <fs_alv> INDEX ls_rowindex-index.
*      LOOP AT <grid_alv> INTO <fs_alv2>.
*        IF <fs_alv2>-sil IS INITIAL AND <fs_alv2>-banfn = <fs_alv>-banfn.
*          ls_items_to_delete-preq_item = <fs_alv2>-bnfpo.
*          ls_items_to_delete-delete_ind = 'X'.
*          ls_items_to_delete-closed = ' '.
*          APPEND ls_items_to_delete TO lt_items_to_delete.
*        ENDIF.
*      ENDLOOP.
*      IF sy-subrc EQ 0 AND lt_items_to_delete IS NOT INITIAL.
*
*        CALL FUNCTION 'BAPI_REQUISITION_DELETE' DESTINATION 'NONE'
*          EXPORTING
*            number                      = <fs_alv>-banfn
...
*        IF sy-subrc EQ 0.
**          <fs_alv>-sil = 'S'.
**          MODIFY <grid_alv> FROM <fs_alv> TRANSPORTING sil WHERE banfn = <fs_alv>-banfn.
*          CALL FUNCTION 'BAPI_TRANSACTION_COMMIT' DESTINATION 'NONE'
*            EXPORTING
*              wait = 'X'.
*        ELSE.
...
*     DELETE FROM <grid_alv> where sil eq 'X'.
...
ENDFORM.                    " DELLINE_1903
*&---------------------------------------------------------------------*
*&      Form  UPDATE_EBAN_BADAT
*----------------------------------------------------------------------*
FORM update_eban_badat  USING    is_talep TYPE zsog_mm_007_s_talep
                        CHANGING cs_eban  TYPE gtt_eban.
  ...
  CLEAR: lv_number  ,ls_return .

  lv_number = cs_eban-banfn.

  CLEAR : ls_pritem  ,lt_pritem .
  ls_pritem-preq_item  = cs_eban-bnfpo.
  ls_pritem-preq_date  = is_talep-file_date.
  ls_pritem-preq_price = '1'.
  APPEND ls_pritem TO lt_pritem.

  CLEAR:ls_pritemx  ,lt_pritemx .
  ls_pritemx-preq_item  = cs_eban-bnfpo.
...
ENDFORM.                    " UPDATE_EBAN_BADAT
*&---------------------------------------------------------------------*
*&      Form  BUDGET_CONTROL_TRANSFER
*----------------------------------------------------------------------*
FORM budget_control_transfer .
  DATA :lt_budget_data         TYPE TABLE OF zsog_mm_007_s_005,
        lt_budget_control_data TYPE tt_budget_cntrl_data,
        lt_mail_data           TYPE TABLE OF zsog_mm_007_s_005,
        lv_ebeln               TYPE ebeln,
        lv_day                 TYPE p,
        lv_sent_to_all         TYPE boolean.

  IF c_trans = 'X' AND c_contr = 'X'.
    MESSAGE text-010 TYPE 'S' DISPLAY LIKE 'E'.
    LEAVE LIST-PROCESSING.
  ELSEIF c_trans IS INITIAL AND c_contr IS INITIAL.
    MESSAGE text-011  TYPE 'S' DISPLAY LIKE 'E'.
    LEAVE LIST-PROCESSING.
  ENDIF.

  IF c_trans = 'X'.
    CLEAR lt_budget_data.
    PERFORM get_budget_data           CHANGING lt_budget_data.
    CHECK lt_budget_data[] IS NOT INITIAL.

    PERFORM set_budget_data           CHANGING lt_budget_data.
    IF sy-batch = 'X'.
      PERFORM save_budget.
    ENDIF.
  ...
  IF c_contr = 'X'.
    CLEAR lt_budget_control_data.
    PERFORM get_budget_control_data   CHANGING lt_budget_control_data.
    CHECK lt_budget_control_data[] IS NOT INITIAL.


    CLEAR  :lt_mail_data,lv_ebeln.
    PERFORM set_budget_control_data   USING    lt_budget_control_data
                                      CHANGING lt_mail_data
                                               lv_ebeln.


    IF lt_mail_data IS NOT INITIAL.
      APPEND LINES OF lt_mail_data TO gt_mail_data.
      IF sy-batch = 'X'.
    ...
    IF lv_day = 1 AND <grid_alv> IS NOT INITIAL.
      CLEAR lt_mail_data[].
      APPEND LINES OF <grid_alv> TO lt_mail_data[].
      PERFORM create_sender_recipient .
      IF gv_error IS INITIAL.
        PERFORM create_mail_body        USING    lt_mail_data
                                                 lv_day.
        PERFORM send_mail               CHANGING lv_sent_to_all.
      ENDIF.
    ENDIF.
  ENDIF.
  ...
  CALL SCREEN 1903.
ENDFORM.                    " BUDGET_CONTROL_TRANSFER
*&---------------------------------------------------------------------*
*&      Form  GET_BUDGET_DATA
*----------------------------------------------------------------------*
FORM get_budget_data  CHANGING et_budget_data TYPE tt_budget_data .
  DATA: lv_bsart TYPE eban-bsart  VALUE 'ZSG1',
        lv_frgkz TYPE eban-frgkz  VALUE '2',
        lv_frggr TYPE eban-frggr  VALUE 'AA',
        lv_ktopl TYPE skat-ktopl  VALUE 'SGHP',
        lv_waers TYPE eban-waers  VALUE 'EUR'.

  SELECT b~sakto
         d~txt20 AS sakto_desc
         a~matnr
         c~maktx
         a~preis
         a~menge
         ...
         a~lfdat
  FROM eban                   AS a
  INNER JOIN ebkn             AS b ON  b~banfn = a~banfn
                                   AND b~bnfpo = a~bnfpo
  LEFT OUTER JOIN makt        AS c ON  c~matnr = a~matnr
                                   AND c~spras = sy-langu
  LEFT OUTER JOIN skat        AS d ON  d~saknr = b~sakto
                                   AND d~ktopl = lv_ktopl
                                   AND d~spras = sy-langu
  INNER JOIN zsog_mm_007_t_01 AS e ON  e~matnr = a~matnr
  INTO CORRESPONDING FIELDS OF TABLE et_budget_data
     WHERE a~bsart  EQ lv_bsart
       AND a~frgkz  EQ lv_frgkz
       AND a~frggr  EQ lv_frggr
       AND a~waers  EQ lv_waers
       AND b~sakto  IN s_sakto
       AND a~loekz  EQ ''.

ENDFORM.                    " GET_BUDGET_DATA
*&---------------------------------------------------------------------*
*&      Form  SET_BUDGET_DATA
*----------------------------------------------------------------------*
FORM set_budget_data  CHANGING it_budget_data TYPE tt_budget_data.
  TYPES:BEGIN OF ty_sakto_group,
           sakto        TYPE ebkn-sakto,
           period       TYPE zsog_mm_007_t_08-period,
           matnr        TYPE mara-matnr,
           waers        TYPE eban-waers,
           sakto_preis  TYPE eban-preis,
         END OF ty_sakto_group.
  DATA: ls_budget_data  TYPE zsog_mm_007_s_005,
        ls_data         TYPE zsog_mm_007_s_005,
        lt_data         TYPE TABLE OF zsog_mm_007_s_005,
        ls_sakto_grp    TYPE ty_sakto_group,
        lt_sakto_grp    TYPE TABLE OF ty_sakto_group.
  CLEAR:ls_budget_data , ls_data     ,
        ls_sakto_grp   , lt_sakto_grp.

  LOOP AT it_budget_data[] INTO ls_budget_data.

    CLEAR ls_sakto_grp.
    ls_sakto_grp-sakto       = ls_budget_data-sakto.
    ls_sakto_grp-period      = ls_budget_data-lfdat+0(6).
    ls_sakto_grp-matnr       = ls_budget_data-matnr.
    ls_sakto_grp-waers       = ls_budget_data-waers.
    ls_sakto_grp-sakto_preis = ls_budget_data-menge * ls_budget_data-preis.
    COLLECT ls_sakto_grp INTO lt_sakto_grp.

    CLEAR ls_budget_data.
  ENDLOOP.

  SORT lt_sakto_grp[]   BY sakto period matnr waers.
  SORT it_budget_data[] BY sakto lfdat  matnr waers.
  LOOP AT lt_sakto_grp INTO ls_sakto_grp.
    CLEAR ls_budget_data.
    READ TABLE it_budget_data[] INTO ls_budget_data WITH KEY sakto      = ls_sakto_grp-sakto
                                                             lfdat+0(6) = ls_sakto_grp-period
                                                             matnr      = ls_sakto_grp-matnr
                                                             waers      = ls_sakto_grp-waers
                                                             BINARY SEARCH.
    IF sy-subrc = 0.
      ls_data-sakto      = ls_budget_data-sakto     .
      ls_data-period     = ls_budget_data-lfdat+0(6).
      ls_data-sakto_desc = ls_budget_data-sakto_desc.
      ls_data-matnr      = ls_budget_data-matnr     .
      ls_data-maktx      = ls_budget_data-maktx     .
      ls_data-preis      = ls_budget_data-preis     .
      ls_data-menge      = ls_budget_data-menge     .
      ls_data-waers      = ls_budget_data-waers     .
      ls_data-kostl      = ls_budget_data-kostl     .
      ls_data-lfdat      = ls_budget_data-lfdat     .

      ls_data-tot_preis = ls_sakto_grp-sakto_preis.

      APPEND ls_data TO lt_data.
    ENDIF.
    CLEAR :ls_budget_data,ls_sakto_grp.
  ENDLOOP.
  DELETE ADJACENT DUPLICATES FROM lt_data COMPARING sakto period.

  IF lt_data[] IS NOT INITIAL.
    APPEND LINES OF lt_data  TO <grid_alv>.
  ENDIF.
ENDFORM.                    " SET_BUDGET_DATA
*&---------------------------------------------------------------------*
*&      Form  GET_BUDGET_CONTROL_DATA
*----------------------------------------------------------------------*
FORM get_budget_control_data  CHANGING et_budget_control_data TYPE tt_budget_cntrl_data.
  DATA: lv_bsart TYPE ekko-bsart  VALUE 'ZSG1',
        lv_bukrs TYPE ekko-bukrs  VALUE '2425',
        lv_ekorg TYPE ekko-ekorg  VALUE '2425',
        lv_werks TYPE ekpo-werks  VALUE '2425',
        lv_waers TYPE ekko-waers  VALUE 'EUR'.
  ...
  INNER JOIN ekkn             AS c ON  c~ebeln = b~ebeln
                                   AND c~ebelp = b~ebelp
  INNER JOIN zsog_mm_007_t_08 AS e ON  e~sakto = c~sakto
  INNER JOIN zsog_mm_007_t_01 AS f ON  f~matnr = b~matnr
  INTO CORRESPONDING FIELDS OF TABLE et_budget_control_data
     WHERE a~bsart  EQ lv_bsart
       AND a~bukrs  EQ lv_bukrs
       AND a~ekorg  EQ lv_ekorg
       AND a~aedat  GE s_aedat-low
       AND a~waers  EQ lv_waers
       AND c~sakto  IN s_sakto
       AND e~period EQ sy-datum+0(6)
       AND b~loekz  EQ ''.
ENDFORM.                    " GET_BUDGET_CONTROL_DATA
*&---------------------------------------------------------------------*
*&      Form  SET_BUDGET_CONTROL_DATA
*----------------------------------------------------------------------*
FORM set_budget_control_data USING    it_budget_control_data TYPE tt_budget_cntrl_data
                             CHANGING et_mail_data           TYPE tt_budget_data
                                      ev_ebeln               TYPE ebeln.
  TYPES: BEGIN OF ty_sakto_group,
             sakto        TYPE ebkn-sakto,
           ...
           END OF ty_sakto_group.
  DATA: ls_budget_data  LIKE gs_budget_cntrl_data,
        ls_data         TYPE zsog_mm_007_s_005,
        lt_data         TYPE TABLE OF zsog_mm_007_s_005,
        ls_sakto_grp    TYPE ty_sakto_group,
        lt_sakto_grp    TYPE TABLE OF ty_sakto_group.
  ...
  SORT it_budget_control_data BY ebeln DESCENDING.
  LOOP AT it_budget_control_data[] INTO ls_budget_data.
    IF sy-tabix = 1.
      ev_ebeln  = ls_budget_data-ebeln.
    ENDIF.
    CLEAR ls_sakto_grp.
    ls_sakto_grp-sakto   = ls_budget_data-sakto.
    ls_sakto_grp-matnr   = ls_budget_data-matnr.
    ls_sakto_grp-waers   = ls_budget_data-waers.
    ls_sakto_grp-netwr   = ls_budget_data-preis.
    COLLECT ls_sakto_grp INTO lt_sakto_grp.
    CLEAR ls_budget_data.
  ENDLOOP.


  SORT it_budget_control_data[] BY sakto matnr waers.
  LOOP AT lt_sakto_grp[] INTO ls_sakto_grp.

    CLEAR ls_budget_data.
    READ TABLE  it_budget_control_data[] INTO ls_budget_data
                                         WITH KEY sakto = ls_sakto_grp-sakto
                                                  matnr = ls_sakto_grp-matnr
                                                  waers = ls_sakto_grp-waers
                                                  BINARY SEARCH.
    IF sy-subrc = 0.
      ...
      ls_data-maktx      = ls_budget_data-maktx     .
      ls_data-preis      = ls_budget_data-preis .
      ls_data-tot_preis  = ls_budget_data-tot_preis .
      ls_data-menge      = ls_budget_data-menge     .
      ls_data-waers      = ls_budget_data-waers     .
      ls_data-kostl      = ls_budget_data-kostl     .
      ls_data-period     = ls_budget_data-period     .


      ls_data-netwr = ls_sakto_grp-netwr.

      ls_data-kalan_butce =  ls_budget_data-tot_preis - ls_data-netwr.
      IF ls_budget_data-tot_preis > 0.
        ls_data-yuzde       =  ( ls_data-netwr / ls_budget_data-tot_preis ) * 100 .
      ENDIF.

      APPEND ls_data TO lt_data.

      IF ls_data-yuzde > 80.
        APPEND ls_data TO et_mail_data.
      ENDIF.
      CLEAR ls_data.
    ENDIF.
  ENDLOOP.
  DELETE ADJACENT DUPLICATES FROM lt_data COMPARING sakto.

  IF lt_data[] IS NOT INITIAL.
    APPEND LINES OF lt_data  TO <grid_alv>.
  ENDIF.

ENDFORM.                    " SET_BUDGET_CONTROL_DATA
*&---------------------------------------------------------------------*
*&      Form  F_CREATE_SENDER_RECIPIENT
*----------------------------------------------------------------------*
FORM create_sender_recipient.
  DATA :ls_mail_data     TYPE zsog_mm_007_s_005 ,
        lr_sender_mail   TYPE REF TO cl_cam_address_bcs,
        lr_recipient     TYPE REF TO if_recipient_bcs,
        lr_bcs_exception TYPE REF TO cx_bcs,
        lv_recipient     TYPE adr6-smtp_addr VALUE 'ilknur.nacar@prodea.com.tr',
        lv_sender        TYPE adr6-smtp_addr VALUE 'muhasebe@sansgirisim.com',
        BEGIN OF ls_mail,
          mail TYPE adr6-smtp_addr,
        END OF ls_mail,
        lt_mail_address LIKE TABLE OF ls_mail.
*        lt_t16fs TYPE TABLE OF t16fs,
*        ls_t16fs TYPE t16fs.
*        BEGIN OF ls_ref_mail,
*          frgc TYPE t16fs-frgc1,
*        END OF ls_ref_mail,
*        lt_ref_mail LIKE TABLE OF ls_ref_mail.
...
*    AND a~frgsx =  'SN'.
*
*  LOOP AT lt_t16fs INTO ls_t16fs.
*    CLEAR ls_ref_mail.
*    ls_ref_mail-frgc =  ls_t16fs-frgc1.
*    APPEND ls_ref_mail TO lt_ref_mail.
*
*    CLEAR ls_ref_mail.
*    ls_ref_mail-frgc =  ls_t16fs-frgc2.
*    APPEND ls_ref_mail TO lt_ref_mail.
*
*    CLEAR ls_ref_mail.
*    ls_ref_mail-frgc =  ls_t16fs-frgc3.
*    APPEND ls_ref_mail TO lt_ref_mail.
*  ENDLOOP.
*  IF lt_ref_mail[] IS NOT INITIAL.
    SELECT  a~mail_adres AS mail
    FROM zsog_mm_018_t_01 AS a
    INTO TABLE lt_mail_address
*    FOR ALL ENTRIES IN lt_ref_mail[]
*      WHERE  a~frgco =  lt_ref_mail-frgc   .
      WHERE  a~budget_mail =  'X'   .
*  ENDIF.

  IF lt_mail_address[] IS INITIAL.
    gv_error =  'X'.
  ELSE.

    CLEAR gr_send_request.
    "Create send request
    gr_send_request = cl_bcs=>create_persistent( ).
    "Email FROM...
    lr_sender_mail = cl_cam_address_bcs=>create_internet_address( lv_sender ).
    "Set Sender
    gr_send_request->set_sender( lr_sender_mail ).

    LOOP AT lt_mail_address INTO ls_mail.
      lr_recipient = cl_cam_address_bcs=>create_internet_address( ls_mail-mail ).
*      "Add recipient to send request
    ...
    ENDLOOP.
  ENDIF.
ENDFORM.                    " F_CREATE_SENDER_RECIPIENT
*&---------------------------------------------------------------------*
*&      Form  F_CREATE_MAIL_BODY
*----------------------------------------------------------------------*
FORM create_mail_body USING it_mail_data TYPE tt_budget_data
                            iv_day       TYPE p.

  DATA: lv_subject TYPE so_obj_des VALUE 'Bütçe Bildirimi Hk.'.

  DATA: lt_objtxt      TYPE STANDARD TABLE OF solisti1,
        lr_document    TYPE REF TO cl_document_bcs.

  CLEAR lt_objtxt.
  PERFORM prepare_budget_table TABLES lt_objtxt
                               USING  it_mail_data
                                      iv_day.
  ...
  gr_send_request->set_document( lr_document ).
  CALL METHOD gr_send_request->set_document( lr_document ).
ENDFORM.                    " F_CREATE_MAIL_BODY
*&---------------------------------------------------------------------*
*&      Form  F_SEND_MAIL
*----------------------------------------------------------------------*
FORM send_mail CHANGING ev_sent TYPE boolean.
  "Send email
...
*----------------------------------------------------------------------*
FORM save_budget.
  DATA: ls_zsogmm007t08 TYPE zsog_mm_007_t_08,
        lt_zsogmm007t08 TYPE TABLE OF zsog_mm_007_t_08,
        ls_data         TYPE zsog_mm_007_s_005,
        ls_return       TYPE bapiret2,
        lv_dummy        TYPE c,
  ...
  LOOP AT <grid_alv> INTO ls_data.
    MOVE-CORRESPONDING ls_data TO ls_zsogmm007t08.
    APPEND ls_zsogmm007t08 TO lt_zsogmm007t08.
  ENDLOOP.

  MODIFY zsog_mm_007_t_08 FROM TABLE lt_zsogmm007t08 .
  IF sy-subrc = 0.
    CALL FUNCTION 'BAPI_TRANSACTION_COMMIT'
      EXPORTING
        wait = 'X'.

    MESSAGE s009(zsg) INTO lv_dummy.
    CLEAR lt_message.
    PERFORM add_bapiret TABLES lt_message.
  ELSE.
    CLEAR ls_return.
    CALL FUNCTION 'BAPI_TRANSACTION_ROLLBACK'
      IMPORTING
        return = ls_return.
*    ls_data-cellcolor = 'C600'.
*    modify <grid_alv> FROM ls_data TRANSPORTING cellcolor
*                                   where cellcolor ne  'C600'.
...
ENDFORM.                    " SYS_ADD_MESSAG
*&---------------------------------------------------------------------*
*&      Form  SEND_BUDGET_MAIL
*----------------------------------------------------------------------*
FORM send_budget_mail.
  DATA:lv_sent_to_all TYPE boolean,
       lv_dummy       TYPE c,
       lt_message     TYPE bapiret2_tt.

  PERFORM create_sender_recipient.
  IF gv_error IS INITIAL.
    PERFORM create_mail_body        USING    gt_mail_data
                                             ' '.
    CLEAR lv_sent_to_all.
    PERFORM send_mail               CHANGING lv_sent_to_all.
    IF sy-batch NE 'X'.
      IF lv_sent_to_all = 'X' .
        MESSAGE  s023(zsg) INTO lv_dummy.
      ELSE.
        MESSAGE  e024(zsg) INTO lv_dummy.
      ENDIF.
      CLEAR lt_message.
      PERFORM add_bapiret  TABLES lt_message.
      CALL FUNCTION 'FINB_BAPIRET2_DISPLAY'
    ...
    ENDIF.
  ENDIF.
ENDFORM.                    "send_budget_mail
*&---------------------------------------------------------------------*
*&      Form  PREPARE_BUDGET_TABLE
*----------------------------------------------------------------------*
FORM prepare_budget_table  TABLES et_objtxt    STRUCTURE solisti1
                           USING  it_mail_data TYPE tt_budget_data
                                  iv_day       TYPE p.

  DATA: ls_objtxt      TYPE solisti1,
        lt_objtxt      TYPE TABLE OF solisti1,
        ls_mail_data   TYPE zsog_mm_007_s_005,
        lv_tot_preis   TYPE string,
        lv_kalan_butce TYPE string,
        lv_netwr       TYPE string,
        lv_yuzde       TYPE string.

  ls_objtxt-line = '<html>'.
  APPEND ls_objtxt TO lt_objtxt.

  CONCATENATE '<p>' 'Sayın İlgili;' INTO ls_objtxt-line
                                    RESPECTING BLANKS.
  APPEND ls_objtxt TO lt_objtxt.
  ...
  IF iv_day IS INITIAL.
    CONCATENATE '<p>'   text-012     INTO ls_objtxt-line
                                     RESPECTING BLANKS.
  ELSEIF iv_day = 1.
    CONCATENATE '<p>'   text-013     INTO ls_objtxt-line
                                     RESPECTING BLANKS.
  ENDIF.
  APPEND ls_objtxt TO lt_objtxt.
  ...
  CONCATENATE '<p>' 'Teşekkürler.' '</p>' INTO ls_objtxt-line
  RESPECTING BLANKS.
  APPEND ls_objtxt TO lt_objtxt.
  ...
  ls_objtxt-line = '<table style="MARGIN:9px" bordercolor="#000000"'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = ' cellspacing="0" cellpadding="1" width="1000"'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = ' border="1"><tbody>'.
  APPEND ls_objtxt TO lt_objtxt.


  ls_objtxt-line = '<tr><th bgcolor="#C0C0C0">Ana Hesap</th>'."
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = '<th bgcolor="#C0C0C0">Ana Hesap Açıklaması</th>'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = '<th bgcolor="#C0C0C0">Malzeme</th>'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = '<th bgcolor="#C0C0C0">Malzeme Açıklaması</th>'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = '<th bgcolor="#C0C0C0">Masraf yeri</th>'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = '<th bgcolor="#C0C0C0">Toplam Bütçe</th>'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = '<th bgcolor="#C0C0C0">Harcanan Bütçe</th>'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = '<th bgcolor="#C0C0C0">Kalan Bütçe</th>'.
  APPEND ls_objtxt TO lt_objtxt.
  ls_objtxt-line = '<th bgcolor="#C0C0C0">Yüzde</th></tr>'.
  APPEND ls_objtxt TO lt_objtxt.


  LOOP AT it_mail_data[] INTO ls_mail_data.


    CONCATENATE '<tr><td>' ls_mail_data-sakto  '</td>' INTO ls_objtxt-line
                                                           RESPECTING BLANKS.
    APPEND ls_objtxt TO lt_objtxt.
    CONCATENATE '<td>' ls_mail_data-sakto_desc '</td>' INTO ls_objtxt-line
                                                           RESPECTING BLANKS.
    APPEND ls_objtxt TO lt_objtxt.
    CONCATENATE '<td>' ls_mail_data-matnr      '</td>' INTO ls_objtxt-line
                                                           RESPECTING BLANKS.
    APPEND ls_objtxt TO lt_objtxt.
    CONCATENATE '<td>' ls_mail_data-maktx      '</td>' INTO ls_objtxt-line
                                                           RESPECTING BLANKS.
    APPEND ls_objtxt TO lt_objtxt.
    CONCATENATE '<td>' ls_mail_data-kostl      '</td>' INTO ls_objtxt-line
                                                           RESPECTING BLANKS.
    APPEND ls_objtxt TO lt_objtxt.
    CLEAR lv_tot_preis.
    lv_tot_preis = ls_mail_data-tot_preis .
    CONCATENATE '<td>' lv_tot_preis            '</td>'      INTO ls_objtxt-line
                                                            RESPECTING BLANKS.
    APPEND ls_objtxt TO lt_objtxt.

    CLEAR lv_netwr.
    lv_netwr = ls_mail_data-netwr.
    CONCATENATE '<td>' lv_netwr                '</td>' INTO ls_objtxt-line
                                                             RESPECTING BLANKS.

    APPEND ls_objtxt TO lt_objtxt.

    CLEAR lv_kalan_butce.
    lv_kalan_butce = ls_mail_data-kalan_butce.
    CONCATENATE '<td>' lv_kalan_butce          '</td>' INTO ls_objtxt-line
                                                            RESPECTING BLANKS.
    APPEND ls_objtxt TO lt_objtxt.


    CLEAR lv_yuzde.
    lv_yuzde = ls_mail_data-yuzde .
    CONCATENATE '<td>' lv_yuzde                 '</td></tr>' INTO ls_objtxt-line
                                                            RESPECTING BLANKS.

    APPEND ls_objtxt TO lt_objtxt.

  ENDLOOP.
ENDFORM.                    " PREPARE_BUDGET_TABLE
